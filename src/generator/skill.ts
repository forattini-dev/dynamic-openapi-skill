...
import type { GeneratedSkill, GenerateSkillOptions, SkillFile } from './types.js'
...
export async function generateSkill(options: GenerateSkillOptions): Promise<GeneratedSkill> {
  ...
  files.push({
    path: 'SKILL.md',
    content: renderSkillMd({ name, description, spec, baseUrl, grouped, split, includeExamples }),
  })
  ...
  return { name, description, spec, files }
}
