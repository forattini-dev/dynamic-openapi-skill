...
- **Regenerate on CI.** Point the CI at the spec's canonical URL and fail the build if the generated skill differs from the committed one:
  ```bash
  npx dynamic-openapi-skill -s $SPEC_URL --stdout > .claude/skills/petstore/SKILL.md
  git diff --exit-code .claude/skills/petstore/SKILL.md
  ```
